...
async function createUserFromDiscord(db, { discordId, email, name, picture }) {
  const result = await db.collection('users').insertOne({
    discordId, email, name, picture, provider: 'discord', createdAt: new Date()
  });
  return { _id: result.insertedId, discordId, email, name, picture, provider: 'discord', createdAt: new Date() };
}
